...
interface SketchState {
  ...
  /** Flip construction flag on the selected entities (guides, no profile). */
...
let entityCounter = 0;
const nextEntityId = () =>
  `e${Date.now().toString(36).slice(-3)}${(entityCounter++).toString(36)}`;
...
function distanceToEntity(uv: Point2, e: SketchEntity): number {
  const segDist = (a: Point2, b: Point2): number => {
    ...
    const t =
      len2 < 1e-12
        ...
        : Math.max(
            0,
            Math.min(1, ((uv[0] - a[0]) * dx + (uv[1] - a[1]) * dy) / len2),
          );
  ...
  switch (e.type) {
    ...
    case "circle":
      return Math.abs(
        Math.hypot(uv[0] - e.center[0], uv[1] - e.center[1]) - e.radius,
      );
    case "arc": {
      // approximate: radial distance if angle within span, else endpoint dist
      const ang =
        (Math.atan2(uv[1] - e.center[1], uv[0] - e.center[0]) * 180) / Math.PI;
      ...
      return Math.min(
        Math.hypot(uv[0] - s[0], uv[1] - s[1]),
        Math.hypot(uv[0] - en[0], uv[1] - en[1]),
      );
    }
    case "rectangle": {
...
export function arcFrom3Points(
  ...
  const ang = (p: Point2) =>
    ((Math.atan2(p[1] - uy, p[0] - ux) * 180) / Math.PI + 360) % 360;
  const aS = ang(S),
    aE = ang(E),
    aM = ang(M);
...
export const useSketchStore = create<SketchState>((set, get) => ({
  ...
  begin(plane, editFeatureId) {
    ...
    if (editFeatureId) {
      const feature = useEditorStore
        .getState()
        .doc.features.find((f) => f.id === editFeatureId);
    ...
    if (plane.kind === "face" || plane.kind === "datumFeature") {
      // plane basis lives in the kernel — fetch async then activate
      const p = plane;
  ...
  escape() {
    const s = get();
    if (s.pending || s.pending2)
      set({ pending: null, pending2: null, version: s.version + 1 });
    else if (s.tool !== "select")
      set({ tool: "select", version: s.version + 1 });
  },
    ...
    if (s.tool === "line" && a && a > 0) {
      ...
      if (s.cursor) {
        const len = Math.hypot(
          s.cursor[0] - s.pending[0],
          s.cursor[1] - s.pending[1],
        );
        if (len > 1e-6) {
  ...
  removeLast() {
    ...
    set({
      ...
      constraints: s.constraints.filter(
        (c) => !c.entities.includes(removed.id),
      ),
  ...
  toggleConstruction() {
    ...
    set({
      entities: s.entities.map((e) =>
        selected.has(e.id) ? { ...e, construction: !e.construction } : e,
      ),
      version: s.version + 1,
    });
  },
  ...
  finish() {
    ...
    if (s.editingFeatureId) {
      ...
      const count = editor.doc.features.filter(
        (f) => f.type === "sketch",
      ).length;
      editor.addFeatures([
        {
